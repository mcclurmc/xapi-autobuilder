...
SR=$(xe sr-create type=nfs \
                  device-config:server=lork.uk.xensource.com \
                  device-config:serverpath=/mnt/vol0/nfs1/scratch1/kronos-test \
                  name-label=nfs shared=true)
...
xe vm-param-set uuid=$vm PV-args="auto-install/enable=true \
url=http://www.uk.xensource.com/kronos/squeeze_preseed.cfg \
interface=auto netcfg/dhcp_timeout=600 hostname=myvm domain=uk.xensource.com"
xe vm-start uuid=$vm

# Wait till guest installs and acquires an IP address
